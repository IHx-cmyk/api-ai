...
exports.default = kuronekoHandler;
const axios_1 = __importDefault(require("axios"));
async function chatWithGemini(message, sessionId = null) {
    try {
        ...
        if (!cookie) {
            const { headers } = await axios_1.default.post('https://gemini.google.com/_/BardChatUi/data/batchexecute?rpcids=maGuAc&source-path=%2F&bl=boq_assistant-bard-web-server_20250814.06_p1&f.sid=-7816331052118000090&hl=en-US&_reqid=173780&rt=c', 'f.req=%5B%5B%5B%22maGuAc%22%2C%22%5B0%5D%22%2Cnull%2C%22generic%22%5D%5D%5D&', { headers: { 'content-type': 'application/x-www-form-urlencoded;charset=UTF-8' } });
        ...
        const { data } = await axios_1.default.post('https://gemini.google.com/_/BardChatUi/data/assistant.lamda.BardFrontendService/StreamGenerate?bl=boq_assistant-bard-web-server_20250729.06_p0&f.sid=4206607810970164620&hl=en-US&_reqid=2813378&rt=c', new URLSearchParams({ 'f.req': JSON.stringify(payload) }).toString(), {
            headers: {
        ...
        return { status: true, response: text, session: newSessionId };
    }
    catch (error) {
        throw new Error(`Gagal mengambil respon dari AI: ${error?.message || "Unknown error"}`);
    }
}
async function kuronekoHandler(req, res) {
    ...
    try {
        const result = await chatWithGemini(q, session);
        res.json(result);
    }
    catch (error) {
