...
const svara = {
  ...
  getVoiceId(vn: string) {
    return this.voiceX[vn.toLowerCase().trim()];
  }
};

export default async function svaraHandler(req: Request, res: Response) {
  ...
  try {
    const response = await axios.post(
      `${svara.api.base}${svara.api.endpoint.generate}`,
      { customerId: ciu, text, voice: voiceId },
      { headers: { ...svara.headers, authorization: toket } }
    );
