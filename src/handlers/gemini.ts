...
async function chatWithGemini(message: string, sessionId: string | null = null) {
    try {
        ...
        if (!match || match.length < 4) throw new Error("Gagal parsing response dari Google (Structure mismatch).");
        ...
        throw new Error(`Gagal mengambil respon dari AI: ${(error as any)?.message || "Unknown error"}`);
    }
}
